...
def dayOfProgrammer(year):
    ...
    #               Ja Fe Ma Ap My Jn Jl Ag Se Oc No De
    ...
    for i, days in enumerate(daysPerMonth):
        programmerDay -= days
        if programmerDay < daysPerMonth[i+1]:
            day = programmerDay
            month = i+2
            break
    ...
    return dayStr + '.' + monthStr + '.' + str(year)
...
if __name__ == '__main__':
    ...
    assert result == '13.09.2017'

    result = dayOfProgrammer(2016)
    assert result == '12.09.2016'

    result = dayOfProgrammer(1800)
    assert result == '12.09.1800'

    result = dayOfProgrammer(1918)
    assert result == '26.09.1918'
